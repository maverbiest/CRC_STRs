...
class DetectorPhobosMaxOld(repeat_detection_run.DetectorPhobos):        
    def phobos_get_repeats(self, infile):
        """ Read repeats from a PHOBOS output file stream successively.
        Read repeats from a PHOBOS output file stream successively.
        Postcondition: infile points to EOF.
        ...
        pattern_begin = re.compile(r"(\d+) :\s+\d")
        # pattern_seq = re.compile(r"([\-ACGT]+)") # <- old pattern, wrongly matches things like '7-nucleotide'
        pattern_seq = re.compile(r"(?<![0-9])([\-ACGT]+)") # <- new pattern, added negative lookbehind for number
        pattern_actual_tr = re.compile(r"(?<![0-9])([\-ACGTN]+)")
        pattern_insertions = re.compile(r"(?:\(([0-9]+),I\))")
        ...
        for i, line in enumerate(infile):
            ...
            if 1 == state:  # Find TR offset
                search = pattern_begin.search(line)
                if search and search.groups()[0] is not None:
                    # print(" *Found tandem repeat begin")     
                    # Find TR begin in sequence, init RepeatRegion               
                    region = repeat_detection_io.RepeatRegion()
                    region.begin = int(search.groups()[0])
                    region.msa = []
                    ...
                    unit = unit_match.groups()[0]
                    if not unit:
                        # print(" *(2->1)Could not find consensus unit for tandem repeat")    
                        state = 1
                    # print(" *(1->2) Found tandem repeat consensus unit")
                    # Update state
                    state = 2

            elif 2 == state:  # Find potential insertions
                insertion_sites = [int(i) for i in pattern_insertions.findall(line)]
                # print(" *(2->3) Checked insertion sites")
                state = 3
                ...
                match = pattern_actual_tr.search(line)
                if match and match.groups()[0] is not None:
                    # print(" *(3->4) Found actual repeat region")
                ...
                match = pattern_seq.search(line)
                if match and match.groups()[0] is not None:
                    # print(" *Found ideal repeat region, start mapping units")
                    # region.msa.append(match.groups()[0])
                    ideal_tr = match.groups()[0]

                    # get locations of unit start and end sites
                    unit_locations_raw = self.map_full_units(ideal_tr, unit, insertion_sites)
                    trimmed_unit_locations = self.trim_unit_locations(unit_locations_raw, len(unit), len(actual_tr))        
                    # extract units from TR to construct msa                
                    msa_list = [actual_tr[i[0]:i[1]+1] for i in trimmed_unit_locations]
                    
                    # print(" *(4->1) Repeat region finished, yielding.")
                    state = 1                
                    if len(msa_list) >= 2:
                        region.msa = msa_list
                        yield region
    ...
    def map_full_units(self, ideal_tr, unit, inserts):
        """ Map rough unit begin and start sites on repeat region
        Start by finding a match for the consensus TR unit in ideal TR representation provided by PHOBOS.
        Starting from this unit, generate a left and a right flank of units until whole repeat region is covered. Merge 
        these three components and return as one list(tuple(int(begin), int(end)), ...)
        This will likely contain units that extend beyond the region begin and end, these will be trimmed later
        using trimmed_unit_locations(). Unit begin and start sites for left and right flank will be corrected for
        insertions using correct_inserts_left() and correct_inserts_right(), respectively.

        Parameters
        ideal_tr (str):     String representation of the ideal repeat region, extracted from PHOBOS output
        unit (str):         String representation of consensus TR unit, extracted from PHOBOS output
        inserts (list):     List of integers specifying where in the repeat region insertions have occurred,
                            extracted from PHOBOS output

        Returns
        [*left_flank, first_match, *right_flank]:
                            Constructs a list of tuples to return. Each tuple contains the begin 
                            and start sites of a tandem repeat unit, corrected for insertions: list(tuple(int(), int()), ... )
        """        
        region_l = len(ideal_tr)
        unit_l = len(unit)
        i = 0
        ...
        # initialize and populate left flank
        left_flank = [(first_match[0] - unit_l, first_match[1] - unit_l)]
        while left_flank[0][1] >= 0:
            new_unit =(
                left_flank[0][0] - unit_l,
        ...
        # Modify unit begin and end sites to correct for insertions
        if inserts:
            left_flank = self.correct_inserts_left(left_flank, inserts)
            right_flank = self.correct_inserts_right(right_flank, inserts)
        return [*left_flank, first_match, *right_flank]
    ...
    def correct_inserts_left(self, left_flank, insertion_sites):  
        ...
        Parameters
        left_flank (list):      List of tuples where each tuple contains the uncorrected begin and end position of a TR unit
                                in the repeat region
        insertion_sites (list):
                                List of integers specifying where in the repeat region insertions have occurred

        Returns
        left_flank (list):      List of tuples where each tuple contains the corrected begin and end position of a TR unit
                                in the repeat region
        """      
        for insert in insertion_sites:
            if insert > left_flank[-1][1]:
                # insert is in right flank, skip
                continue
            updated_flank = []
            for unit in left_flank:
                if insert >= unit[0] and insert <= unit[1]:
                    # Insert in unit: push begin back one position
                    updated_flank.append((unit[0] - 1, unit[1]))
                if insert > unit[1]:
                    # Insert ahead of unit, push both begin and end back one position
                    updated_flank.append((unit[0] - 1, unit[1] - 1))
                if insert < unit[0]:
                    # Insert not in or ahead of unit, do not modify
                    updated_flank.append(unit)
            left_flank = updated_flank        
        return left_flank
    ...
    def correct_inserts_right(self, right_flank, insertion_sites):
        ...
        Parameters
        right_flank (list):     List of tuples where each tuple contains the uncorrected begin and end position of a TR unit
                                in the repeat region
        insertion_sites (list):
                                List of integers specifying where in the repeat region insertions have occurred

        Returns
        right_flank (list):     List of tuples where each tuple contains the corrected begin and end position of a TR unit
                                in the repeat region
        """
        for insert in insertion_sites:
            if insert < right_flank[0][0]:
                # insert is in left flank, skip
                continue
            updated_flank = []
            for unit in right_flank:
                if insert >= unit[0] and insert <= unit[1]:
                    # Insert in unit: push end up one position
                    updated_flank.append((unit[0], unit[1] + 1))
                if insert < unit[0]:
                    # Insert prior to unit, push both begin and end up one position
                    updated_flank.append((unit[0] + 1, unit[1] + 1))
                if insert > unit[1]:
                    # Insert not in or prior to unit, do not modify
                    updated_flank.append(unit)
            right_flank = updated_flank                 
        return right_flank


    def trim_unit_locations(self, raw_units, unit_length, region_length):
        """ Correct units that overlap repeat region begin or end (e.g. (-3, 4) -> (0, 4)) 
        and trim units that fall out of the repeat region range (e.g. remove (-10, -3))

        Parameters
        raw_units (list):       List of tuples where each tuple contains the begin and end position of a TR unit
                                in the repeat region. Unit positions should have been corrected for insertions by the
                                appropriate functions previously
        unit_length (int):      Length of the consensus repeat unit
        region_length (int):    Length of the repeat region

        Returns
        trimmed_units (list):   List of tuples where each tuple contains the begin and end position of a TR unit
                                in the repeat region. Out of range tuples will have been removed and begin/end overlaps
        ...
        for unit in raw_units:
            if unit[1] < 0:
                ...
                trimmed_units.append((0, unit[1]))
                continue
            elif unit[0] >= region_length:
                continue
            elif unit[1] > region_length:
                trimmed_units.append((unit[0], region_length))
                continue
            else:
                trimmed_units.append(unit)
        return trimmed_units

def main():    
    detector = DetectorPhobosMax()
    # detector = DetectorPhobosMaxOld()
    detector.config.valopts["--reportUnit"] = 0
    detector.config.valopts["--maxUnitLen"] = 15

    input_file = "/cfs/earth/scratch/verb/projects/CRC_STRs/results/test/test_phobos/PHOBOS/phobos.o"
